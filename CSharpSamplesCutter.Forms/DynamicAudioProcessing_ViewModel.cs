...
namespace CSharpSamplesCutter.Forms
{
	internal class DynamicAudioProcessing_ViewModel
	{
		...
		internal DynamicAudioProcessing_ViewModel(Form window, ComboBox comboBox_functions, Button button_go, Panel panel_Parameters, CheckBox checkBox_autoParameters, CheckBox checkBox_optionalParameters, float inputWidthRatio = 0.4f)
		{
		...
		private void BuildParametersPanel(string methodName)
		{
			try
			{
				...
				for (int i = 0; i < parameters.Length; i++)
				{
					...
					var labelText = $"{p.Name} ({this.GetPrimitiveTypeNameFromParameterType(p.ParameterType)}{(p.IsOptional ? " = optional" : "")})";
					var lbl = new Label()
					{
						Text = labelText,
		...
		private async Task InvokeSelectedMethodAsync()
		{
			...
			{
				this.Button_go.Invoke((Action)(() => this.Button_go.Enabled = true));
			}
		...
		private string GetPrimitiveTypeNameFromParameterType(Type parameterType)
		{
